...
#if defined(NDEBUG)
# error "Crowncash cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1403fbaf434e8d00f99a95535d2e4feaaed9cce9f198cdd572c51d9992dcda6f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Crowncash: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Crowncash Signed Message:\n";
...
{
    // Crowncash: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Crowncash: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Crowncash: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1526000447, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "palhaço na Lua nunca mais";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04cf105c4375c22035e3c43001877211cc867dafc28a872b821cfe9c1efd6f028106266531b157346bee3512e3d40fbbb4ff0c147b6c7197cd5fab70cb256afd02") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1539606071;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3798408;

        if (fTestNet)
        {
            block.nTime    = 1539598207;
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x046c36b1d11cd046c6d63e7244bbfd45ae227e7a9ab1ee21dd508c28f9380c7e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd4, 0xac, 0xc6, 0x03 }; // Crowncash: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CrowncashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CrowncashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CrowncashMiner(CWallet *pwallet)
{
    printf("CrowncashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("crowncash-miner");
    ...
    try { loop {
        ...
        printf("Running CrowncashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CrowncashMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CrowncashMiner, pwallet));
}
